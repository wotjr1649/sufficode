...
6. Derive the record-introduction commit with `git log --diff-filter=A --format=%H -- <path>` and require exactly one reachable result. Require that commit to have exactly one parent, require `Observed commit` to equal its first parent, and require the introduction commit to be an ancestor of current HEAD. Never use the latest commit that modified the path as the record-introduction commit. Validate the current tracked, unmodified record bytes separately against `HEAD:<path>`. Treat a broken relation after rebase or history rewrite as stale.
...
- `Observed commit` is HEAD immediately before the record draft and the base of task-scoped staging. The unique record-introduction commit must have it as first parent; this is the commit that added the exact tracked record path, not the latest commit that modified it.
